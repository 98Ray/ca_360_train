...
def generate_action(env, state_list, policy, action_bound):
    if env.index == 0:
        ...
        s_list = Variable(torch.from_numpy(s_list)).squeeze(0).float().cuda()
        ...
        v, a, logprob, mean = policy(s_list.squeeze(1), goal_list, speed_list)
...
def generate_action_no_sampling(env, state_list, policy, action_bound):
    ...
    _, _, _, mean = policy(s_list.squeeze(1), goal_list, speed_list)
